...
For identifiability a sum-to-zero constraint is imposed on the components of the scaled iCAR/BYM model associated with each connected subgraph of the matrix $R$ \cite{Riebler2016}.
...
f_{SPACE}(z_s,g_s) = \frac{1}{\sqrt{\tau_{f_{\text{SPACE}}}}}\left(\sqrt{1-\phi_{f_{\text{SPACE}}}}v(z_s,g_s)+\phi_{f_{\text{SPACE}}} u(z_s,g_s)\right)\\
v(z_s,g_s) \sim \mathcal{N}\left(0,1\right) \\
u(g_s) \sim \mathcal{N}\left(0,R_*^{-}\right) \\
\end{cases}\\
\mathrm{Corr}(u(z_s, 1),u(z_s, 2)) =\eta_{f_{\text{SPACE}}}
\end{cases}\\
\mathrm{Hyperparameters}
\begin{cases}
\eta_{f_{\text{SPACE}}}=  \frac{\exp(\eta_{f_{\text{SPACE}}}^*) - 1}{\exp(\eta_{f_{\text{SPACE}}}^*) + 2 - 1}\\
...
\end{cases}\\
R_*:~\text{scaled counterpart of the matrix R}
\end{cases}
\end{cases}
\end{cases}
\end{equation*}
