...
function_bru_elements <- function(prec_fixed,
    ...
    if (!str_detect(typespace, "bym2")) {
      ...
      cmp <- update(
        ...
        ~ . +
          fspace(
            idspace,
            values = unique(data_cp_final$idspace),
            group = virusgroup,
            ngroup = 2,
            model = "bym2",
            graph = W,
  ...
  else if (str_detect(typespace, "spacetime") &
           str_detect(typespace, "bym2")) {
    cmp <- update(
      cmp,
      ~ . +
        fspacetime(
        ...
        copyfspacetime(
          ...
          hyper = list(beta = list(
            fixed = TRUE, initial = 1
          ))
        ) +
        copyfspacetimeopportunistic(
          idspace,
          weights = (virusgroup - 1) * opportunistic,
          group = idtime,
          replicate = 1,
          copy = "fspacetimeopportunistic",
          hyper = list(beta = list(
            fixed = TRUE, initial = 1
          ))
        )
...
#
# x <- carto_city %>%
#   filter(as.numeric(str_sub(code_postal, 1, 2))<97) %>%
#   left_join(idcentroids %>%
#               st_drop_geometry(), by = "code_postal") %>%
